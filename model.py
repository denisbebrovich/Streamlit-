...
st.title('Сервис Суммаризации Текста (Text Summarization)')
...
st.write("1. Приложение загружает предобученную нейросеть (модель) для суммаризации текста из библиотеки Hugging Face Transformers.")
st.write("2. Вы вставляете любой достаточно длинный текст в текстовое поле.")
st.write("3. Модель анализирует ваш текст и генерирует его краткое содержание на основе параметров `min_length` и `max_length`.")
st.write("4. **Важно:** При первом запуске загрузка модели может занять некоторое время и потребовать значительного объема памяти, особенно для более крупных моделей.")
